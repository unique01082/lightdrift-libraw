...
async function main() {
  ...
  if (singleResult.success) {
    ...
    console.log(`  Dimensions: ${singleResult.dimensions.width}x${singleResult.dimensions.height}`);
  ...
  poolResults.forEach((result, index) => {
    if (result.success) {
      console.log(`  ${formats[index].toUpperCase()}: ${(result.fileSize / 1024).toFixed(2)} KB (${result.processingTimeMs} ms)`);
    }
  });
