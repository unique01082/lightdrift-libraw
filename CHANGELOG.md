...
- Promoted the RC.2 SDK contract to stable without changing its JavaScript,
  TypeScript, native, or legacy compatibility surfaces.
- Published the supported Node.js 22/24 package under npm's default `latest`
  dist-tag.
- Retained prebuilt binaries for Linux glibc x64/arm64, macOS x64/arm64, and
  Windows x64, plus the vendored source-build fallback.
- Preserved SLSA provenance, CycloneDX SBOM, sanitizer, package-content, and
  clean ESM/CommonJS consumer gates for the stable publication.
...
- Rebuilt the README as a beginner-friendly landing page with representative
  ...
  boundaries.
- Reorganized the documentation hub, getting-started guide, and format guide
  around user tasks.
- Labeled historical beta examples so new users enter through the stable API.
...
- Refreshed npm package metadata and RC.2 release notes.
