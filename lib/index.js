    ...
    return new Promise(async (resolve, reject) => {
      try {
          ...
          switch (format) {
            ...
            case 'png':
              result = await this.createPNGBuffer({
                width: options.maxSize,
                compressionLevel: options.compressionLevel !== undefined ? options.compressionLevel : 6
              });
              break;
            case 'webp':
        ...
        resolve({
          ...
          dimensions: result.metadata.dimensions || result.metadata.originalDimensions,
