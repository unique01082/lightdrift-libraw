...
async function analyzeMemoryLeaks(snapshots) {
  ...
  console.log(
    `  External:   ${externalGrowth > 0 ? "+" : ""}${externalGrowth} MB`
  );
  ...
  console.log(
    `Second half growth (${midpoint.iteration}-${last.iteration}): ${secondHalfGrowth} MB`
  );
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Average time per iteration: ${(totalTime / result.iterations).toFixed(
        2
      )} ms`
    );
